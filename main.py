...
if __name__ == '__main__':
    ...
    if helper.params['is_poison']:
        logger.info(f"后门C端参与者: {(helper.params['total_list'])}")
    ...
    logger.info(f"实验数据集:  {helper.params['environment_name']}")
        ...
        if helper.params['is_random_namelist']:
            if helper.params['is_random_adversary']:  # random choose , maybe don't have advasarial
                ...
                for _name_keys in agent_name_keys:
                    if _name_keys in helper.params['total_list']:
                ...
                for idx in range(0, len(helper.params['total_list'])):
                    for ongoing_epoch in ongoing_epochs:
                        if ongoing_epoch in helper.params[str(idx) + '_poison_epochs']:
                            if helper.params['total_list'][idx] not in adversarial_name_keys:
                                adversarial_name_keys.append(helper.params['total_list'][idx])

                nonattacker = []
                for adv in helper.params['total_list']:
            ...
            if helper.params['is_random_adversary'] == False:
                adversarial_name_keys = copy.deepcopy(helper.params['total_list'])
        logger.info(f'当前轮次:{epoch} 本轮的参与方 : {agent_name_keys}.')
        epochs_submit_update_dict, num_samples_dict = ImageTrain(helper=helper, start_epoch=epoch,
        ...
        if helper.params['is_poison']:
            ...
            if helper.params['vis_trigger_split_test']:
                helper.target_model.trigger_agent_test_vis(vis=vis, epoch=epoch, acc=epoch_acc_p, loss=None,
                                                           eid=helper.params['environment_name'],
                                                           name="global_combine")
           # if len(helper.params['total_list']) == 1:  # centralized attack
            #    if helper.params['centralized_test_trigger'] == True:  # centralized attack test on local triggers
             ...
             #   for agent_name_key in helper.params['total_list']:
    ...
    logger.info(f"完成时间: {helper.params['current_time']}. "
                f"Visdom 环境名: {helper.params['environment_name']}")
